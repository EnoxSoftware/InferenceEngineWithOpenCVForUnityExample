using System;
using InferenceEngineWithOpenCVForUnity;
using InferenceEngineWithOpenCVForUnity.Face;
using InferenceEngineWithOpenCVForUnity.Hand;
using InferenceEngineWithOpenCVForUnity.Pose;
...
namespace InferenceEngineWithOpenCVForUnityExample
{
    ...
    public class MultiSource2MatHelperExample : MonoBehaviour
    {
        ...
        /// <summary>
        /// Toggle for enabling/disabling face detection.
        /// </summary>
        public Toggle EnableFaceDetectionToggle;

        /// <summary>
        /// Toggle for enabling/disabling hand detection.
        /// </summary>
        public Toggle EnableHandDetectionToggle;

        /// <summary>
        /// Toggle for enabling/disabling pose detection.
        /// </summary>
        public Toggle EnablePoseDetectionToggle;

        [Space(10)]

        /// <summary>
        /// Toggle for using best score only in inference engines.
        /// </summary>
        public Toggle UseBestScoreOnlyToggle;

        [Space(10)]

        public InferenceEngineFaceDetector InferenceEngineFaceDetecter;

        public InferenceEngineHandDetector InferenceEngineHandDetecter;

        public InferenceEnginePoseDetector InferenceEnginePoseDetecter;
        ...
        private InferenceEngineFaceDetector.EstimationData[] _detectedFaceEstimationData = null;

        private InferenceEngineHandDetector.EstimationData[] _detectedHandEstimationData = null;

        private InferenceEnginePoseDetector.EstimationData[] _detectedPoseEstimationData = null;
...
#if UNITY_WEBGL
            // Disable useBestScoreOnlyToggle operation when running on WebGL
            UseBestScoreOnlyToggle.interactable = false;
#endif

            // Initialize inference engines
            await InitializeInferenceEngines();
        ...
        private void Update()
        {
            ...
            if (_multiSource2MatHelper.IsPlaying() && _multiSource2MatHelper.DidUpdateThisFrame())
            {
                ...
                if (EnableFaceDetectionToggle != null && EnableFaceDetectionToggle.isOn)
                {
                    if (InferenceEngineFaceDetecter.CanStartInference())
                    {
                        _ = InferenceEngineFaceDetecter.InferAsync(rgbaMat);
                    }

                    // Visualize face detection results
                    if (_detectedFaceEstimationData != null)
                    {
                        InferenceEngineFaceDetector.Visualize(rgbaMat, _detectedFaceEstimationData, false, true);
                    }
                }

                // Hand detection processing
                if (EnableHandDetectionToggle != null && EnableHandDetectionToggle.isOn)
                {
                    if (InferenceEngineHandDetecter.CanStartInference())
                    {
                        _ = InferenceEngineHandDetecter.InferAsync(rgbaMat);
                    }

                    // Visualize hand detection results
                    if (_detectedHandEstimationData != null)
                    {
                        InferenceEngineHandDetector.Visualize(rgbaMat, _detectedHandEstimationData, false, true);
                    }
                }

                // Pose detection processing
                if (EnablePoseDetectionToggle != null && EnablePoseDetectionToggle.isOn)
                {
                    if (InferenceEnginePoseDetecter.CanStartInference())
                    {
                        _ = InferenceEnginePoseDetecter.InferAsync(rgbaMat);
                    }

                    // Visualize pose detection results
                    if (_detectedPoseEstimationData != null)
                    {
                        InferenceEnginePoseDetector.Visualize(rgbaMat, _detectedPoseEstimationData, false, true);
                    }
                }
        ...
        {
            // Dispose inference engines
            await DisposeInferenceEngines();
        ...
        /// <summary>
        /// Receives OnDetectFinished event from InferenceEngineFaceDetector.
        /// </summary>
        /// <param name="data">The face detection results.</param>
        public void OnFaceDetectFinished(InferenceEngineFaceDetector.EstimationData[] data)
        {
            _detectedFaceEstimationData = data;
        }

        /// <summary>
        /// Receives OnDetectFinished event from InferenceEngineHandDetector.
        /// </summary>
        /// <param name="data">The hand detection results.</param>
        public void OnHandDetectFinished(InferenceEngineHandDetector.EstimationData[] data)
        {
            _detectedHandEstimationData = data;
        }

        /// <summary>
        /// Receives OnDetectFinished event from InferenceEnginePoseDetector.
        /// </summary>
        /// <param name="data">The pose detection results.</param>
        public void OnPoseDetectFinished(InferenceEnginePoseDetector.EstimationData[] data)
        {
            _detectedPoseEstimationData = data;
        }

        /// <summary>
        /// Raises the enable face detection toggle value changed event.
        /// </summary>
        public void OnEnableFaceDetectionToggleValueChanged()
        {
            if (EnableFaceDetectionToggle != null && !EnableFaceDetectionToggle.isOn)
            {
                _detectedFaceEstimationData = null;
            }
        }

        /// <summary>
        /// Raises the enable hand detection toggle value changed event.
        /// </summary>
        public void OnEnableHandDetectionToggleValueChanged()
        {
            if (EnableHandDetectionToggle != null && !EnableHandDetectionToggle.isOn)
            {
                _detectedHandEstimationData = null;
            }
        }

        /// <summary>
        /// Raises the enable pose detection toggle value changed event.
        /// </summary>
        public void OnEnablePoseDetectionToggleValueChanged()
        {
            if (EnablePoseDetectionToggle != null && !EnablePoseDetectionToggle.isOn)
            {
                _detectedPoseEstimationData = null;
            }
        }

        /// <summary>
        /// Raises the use best score only toggle value changed event.
        /// </summary>
        public async void OnUseBestScoreOnlyToggleValueChanged()
        {
            if (UseBestScoreOnlyToggle != null)
            {
                // Dispose inference engines first
                await DisposeInferenceEngines();

                // Update useBestFaceOnly flags for all inference engines
                if (InferenceEngineFaceDetecter != null)
                {
                    InferenceEngineFaceDetecter.UseBestFaceOnly = UseBestScoreOnlyToggle.isOn;
                }
                if (InferenceEngineHandDetecter != null)
                {
                    InferenceEngineHandDetecter.UseBestHandOnly = UseBestScoreOnlyToggle.isOn;
                }
                if (InferenceEnginePoseDetecter != null)
                {
                    InferenceEnginePoseDetecter.UseBestPoseOnly = UseBestScoreOnlyToggle.isOn;
                }

                // Reinitialize inference engines with new settings
                await InitializeInferenceEngines();
            }
        }
        ...
        /// </summary>
        private async Awaitable InitializeInferenceEngines()
        {
#if UNITY_WEBGL
            // Set useBestFaceOnly to true for WebGL platform
            if (InferenceEngineFaceDetecter != null)
            {
                InferenceEngineFaceDetecter.UseBestFaceOnly = true;
            }
            if (InferenceEngineHandDetecter != null)
            {
                InferenceEngineHandDetecter.UseBestHandOnly = true;
            }
            if (InferenceEnginePoseDetecter != null)
            {
                InferenceEnginePoseDetecter.UseBestPoseOnly = true;
            }
#endif

            // Initialize face detection engine
            await InferenceEngineFaceDetecter.InitializeAsync(CancellationToken.None);

            // Initialize hand detection engine
            await InferenceEngineHandDetecter.InitializeAsync(CancellationToken.None);

            // Initialize pose detection engine
            await InferenceEnginePoseDetecter.InitializeAsync(CancellationToken.None);
        }

        /// <summary>
        /// Dispose all inference engines.
        /// </summary>
        private async Awaitable DisposeInferenceEngines()
        {
            await InferenceEngineFaceDetecter.DisposeAsync();
            await InferenceEngineHandDetecter.DisposeAsync();
            await InferenceEnginePoseDetecter.DisposeAsync();
        ...
        private void ReleaseResources()
        {
            ...
            if (_graphicsBuffer != null) { _graphicsBuffer.Dispose(); _graphicsBuffer = null; }
        }
    }
}
