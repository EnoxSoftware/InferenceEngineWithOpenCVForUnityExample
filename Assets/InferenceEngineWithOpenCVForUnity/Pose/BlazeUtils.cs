...
namespace InferenceEngineWithOpenCVForUnity.Pose
{
    ...
    public static class BlazeUtils
    {
        ...
        /// <summary>
        /// Filters and selects the best poses using NMS (Non-Maximum Suppression)
        /// </summary>
        /// <param name="rawBoxes">Raw detection boxes tensor</param>
        /// <param name="rawScores">Raw detection scores tensor</param>
        /// <param name="anchors">Anchor boxes tensor</param>
        /// <param name="inputSize">Input size for coordinate conversion</param>
        /// <param name="iouThreshold">IoU threshold for NMS</param>
        /// <param name="scoreThreshold">Score threshold for filtering</param>
        /// <returns>Tuple containing selected indices, scores, and boxes</returns>
        public static (FunctionalTensor, FunctionalTensor, FunctionalTensor) NMSFiltering(FunctionalTensor rawBoxes, FunctionalTensor rawScores, FunctionalTensor anchors, float inputSize, float iouThreshold, float scoreThreshold)
        {
            ...
            var selectedBoxes = Functional.IndexSelect(rawBoxes, 1, selectedIndices).Unsqueeze(0); // (1, N, 12)
